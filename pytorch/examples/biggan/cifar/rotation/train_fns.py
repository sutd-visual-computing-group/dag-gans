    ...
    def train(x, y):
        ...
        for step_index in range(config['num_D_steps']):
            ...
            for accumulation_index in range(config['num_D_accumulations']):
                z_.sample_()
                y_.sample_()
                D_scores, D_scores_rotate90, D_scores_fliph, D_scores_flipv, \
                D_scores_croptl, D_scores_croptr, D_scores_cropbl, D_scores_cropbr = GD(z_[:config['batch_size']], y_[:config['batch_size']],
                ...
                if config['CR'] > 0:
                    ...
                    D_fake_fliph, D_real_fliph = D_scores_fliph
                    D_fake_flipv, D_real_flipv = D_scores_flipv
                    # cropping
                ...
                # rotation
                D_loss_real_rotate90, D_loss_fake_rotate90 = losses.discriminator_loss(D_fake_rotate90, D_real_rotate90)
                D_loss_real_fliph, D_loss_fake_fliph = losses.discriminator_loss(D_fake_fliph, D_real_fliph)
                D_loss_real_flipv, D_loss_fake_flipv = losses.discriminator_loss(D_fake_flipv, D_real_flipv)
                # croping
                D_loss_real_croptl, D_loss_fake_croptl = losses.discriminator_loss(D_fake_croptl, D_real_croptl)
                D_loss_real_croptr, D_loss_fake_croptr = losses.discriminator_loss(D_fake_croptr, D_real_croptr)
                D_loss_real_cropbl, D_loss_fake_cropbl = losses.discriminator_loss(D_fake_cropbl, D_real_cropbl)
                D_loss_real_cropbr, D_loss_fake_cropbr = losses.discriminator_loss(D_fake_cropbr, D_real_cropbr)
                
                D_loss = D_loss_real + D_loss_fake + D_loss_CR
                # rotation
                D_loss_rotate90 = D_loss_real_rotate90 + D_loss_fake_rotate90
                D_loss_fliph = D_loss_real_fliph + D_loss_fake_fliph
                D_loss_flipv = D_loss_real_flipv + D_loss_fake_flipv
                # cropping
                ...
                D_loss = D_loss + lambda_D/4*(D_loss + D_loss_rotate90 + D_loss_fliph + D_loss_flipv) \
        ...
        if not config['fix_G']:
            ...
            for accumulation_index in range(config['num_G_accumulations']):
                z_.sample_()
                y_.sample_()
                D_fake, D_fake_rotate90, D_fake_fliph, D_fake_flipv, \
                        D_fake_croptl, D_fake_croptr, D_fake_cropbl, D_fake_cropbr = GD(z_, y_, train_G=True, policy=config['DiffAugment'])
                
                G_loss_rotate0   = losses.generator_loss(D_fake)           / float(config['num_G_accumulations'])
                # rotation
                G_loss_rotate90  = losses.generator_loss(D_fake_rotate90)  / float(config['num_G_accumulations'])
                G_loss_fliph = losses.generator_loss(D_fake_fliph) / float(config['num_G_accumulations'])
                G_loss_flipv = losses.generator_loss(D_fake_flipv) / float(config['num_G_accumulations'])
                # cropping
                G_loss_croptl  = losses.generator_loss(D_fake_croptl)  / float(config['num_G_accumulations'])
                G_loss_croptr  = losses.generator_loss(D_fake_croptr)  / float(config['num_G_accumulations'])
                G_loss_cropbl  = losses.generator_loss(D_fake_cropbl)  / float(config['num_G_accumulations'])
                G_loss_cropbr  = losses.generator_loss(D_fake_cropbr)  / float(config['num_G_accumulations'])

                G_loss = G_loss_rotate0 + lambda_G/4.*(G_loss_rotate0 + G_loss_rotate90 + G_loss_fliph + G_loss_flipv) \
